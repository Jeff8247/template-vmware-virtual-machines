...
### Changed
- Bumped module source ref to `v1.0.23` — template updates in vCenter no longer trigger VM recreation (`clone[0].template_uuid` is now in `lifecycle.ignore_changes`).
- README: added note to Templates section explaining that template refreshes in vCenter do not cause VM recreation.
...
- Ansible inventory generation via `ansible.tf`. On `terraform apply`, writes a ready-to-use inventory under `inventory/`:
  ...
  - `inventory/group_vars/linux.yml` — SSH on port 22, `ansible_become: true` via sudo; `realm_join_user` sourced from `windows_domain_user` for Ansible-driven Linux domain join
